...
# (optional) build the Materials-tab file list from your local materials/
python3 scripts/gen_materials_manifest.py

# open the dashboard — double-click dashboard/index.html,
# or serve from the REPO ROOT (not from dashboard/) so the Materials links resolve:
python3 -m http.server 8000    # → http://localhost:8000/dashboard/
```
...
The dashboard has two tabs: **Dashboard** (countdowns, checklists, topic finder) and **Materials** (a browsable file tree of everything in `materials/` + `assets/`). The Materials tab reads `dashboard/materials.js`, which is git-ignored and regenerated by `scripts/gen_materials_manifest.py` — re-run it whenever you sync new files.

Some coursework lives as **Moodle quiz activities** rather than downloadable files (e.g. the Analysis II Serien). `scripts/scrape_moodle_quizzes.py` reconstructs those into self-contained HTML (math via MathJax, images embedded) in `materials/`, so they're browsable and searchable like everything else. It reads your local `materials/config.json` token and only *reads* your quiz questions — it never submits an attempt. Example:
